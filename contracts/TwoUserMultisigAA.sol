...
contract TwoUserMultisigAA is IAccount, IERC1271 {
    using TransactionHelper for Transaction;

    error TwoUserMultisigAA__NotBootloader();
    error TwoUserMultisigAA__NotEnoughBalance();
    error TwoUserMultisigAA__TransactionFailed();
    error TwoUserMultisigAA__InvalidSignatureLength();
    error TwoUserMultisigAA__ExecutionFailed();
    error TwoUserMultisigAA__ValidationFailed();
    ...
    modifier onlyBootloader() {
        if (msg.sender != BOOTLOADER_FORMAL_ADDRESS) {
            revert TwoUserMultisigAA__NotBootloader();
        }
        _;
    }
    ...
    function executeTransactionFromOutside(
        ...
        if (magic != ACCOUNT_VALIDATION_SUCCESS_MAGIC) {
            revert TwoUserMultisigAA__ValidationFailed();
        }
        _executeTransaction(_transaction);
    }
    ...
    function payForTransaction(
        ...
        if (!success) {
            revert TwoUserMultisigAA__TransactionFailed();
        }
    }
    ...
    function _executeTransaction(Transaction calldata _transaction) internal {
        ...
        if (to == address(DEPLOYER_SYSTEM_CONTRACT)) {
            ...
            if (!success) {
                revert TwoUserMultisigAA__ExecutionFailed();
            }
        }
    }
    ...
    function extractECDSASignature(
        ...
        if (_fullSignature.length != 130) {
            revert TwoUserMultisigAA__InvalidSignatureLength();
        }
    ...
    function isValidSignature(
        ...
        address recoverAddr1 = ECDSA.recover(_hash, signature1);
        address recoverAddr2 = ECDSA.recover(_hash, signature2);

        // Note, that we should abstain from using the require or revert here in order to allow for fee estimation to work
        if (recoverAddr1 != owner1 || recoverAddr2 != owner2) {
            magic = bytes4(0);
        }
    }
}
